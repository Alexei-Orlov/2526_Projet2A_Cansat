...
/* Fixed ~8 s window: the previous adaptive approach (wait until die temperature
   stabilized, up to 90 s) could freeze the HMI screen for a long time when
   triggered right after config entry. Triggering calibration at CONFIG EXIT
   instead (see startTaskFSM) — after the operator has been in the menu for a
   while — provides a better thermal state without the unbounded wait. */
